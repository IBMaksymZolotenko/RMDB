...
BEGIN TRY
	...
	DECLARE	@STARTCALCDATE	DATETIME,
			@STARTCALCDATE_B2	DATETIME,
			@ENDCALCDATE_B2		DATETIME;

	SET	@STARTCALCDATE = GETDATE();
	...
	--
	SELECT	@INN = T1.Info
	FROM	[Decision].dbo.ConnotationValues T1 WITH (NOLOCK)
	INNER JOIN [Decision].dbo.ConnotationList T2 WITH (NOLOCK) ON T2.ID = T1.Kind
	...
	FROM	(
				...
				FROM	[Decision].[dbo].[CreditBur7_Subject] a1 WITH (NOLOCK)
				left join [Decision].[dbo].[CreditBur7_Contract] a WITH (NOLOCK) on a1.Id = a.SubjectId
				...
				SELECT	U.ROWID,
						...
						END AS closedate,
						u1.dlamt
				FROM	[Decision].[dbo].[CreditBur3_comp] u WITH (NOLOCK)
				LEFT JOIN [Decision].[dbo].[CreditBur3_crdeal] u1 WITH (NOLOCK) ON u.Id = u1.compId
				INNER JOIN	(
								...
								FROM	[Decision].[dbo].[CreditBur3_comp] T1 WITH (NOLOCK)
								INNER JOIN [Decision].dbo.CreditBur3_crdeal T2 WITH (NOLOCK) ON T2.compId = T1.ID
								INNER JOIN [Decision].dbo.CreditBur3_deallife T3 WITH (NOLOCK) ON T3.crdealId = T2.Id
	...
	OUTER APPLY	(
					...
					from	(
								...
								FROM	[Decision].[dbo].[CreditBur7_Subject] a WITH (NOLOCK)
								INNER JOIN [Decision].[dbo].[CreditBur7_Event] a1 WITH (NOLOCK) ON A1.SubjectId = A.ID
																								AND event = 'request'
								...
								SELECT	u.rowid,
										u1.redate as redate
								FROM	[Decision].[dbo].[CreditBur3_comp] u WITH (NOLOCK)
								INNER JOIN [Decision].[dbo].[CreditBur3_credres] u1 WITH (NOLOCK) ON U1.compId = U.Id
	...
	OUTER APPLY	(
					SELECT	TOP 100
							*
					FROM	[Decision].[dbo].[CreditBur3_credres] u1 WITH (NOLOCK)
	...
	OUTER APPLY	(
					SELECT	TOP 10
							...
							m.[dlds],
							u1.dldonor
					FROM	[Decision].[dbo].[CreditBur3_crdeal] u1 WITH (NOLOCK)
					OUTER APPLY	(
									SELECT	TOP 1
											[dlref],
											[dlds]
									FROM	[Decision].[dbo].[CreditBur3_deallife] tt WITH (NOLOCK)
	...
	from	(
				SELECT	a1.rowid,
						...
						ISNULL([totalAmount], [creditLimit]) AS credamount
				FROM	[Decision].[dbo].[CreditBur7_Subject] a1 WITH (NOLOCK)
				LEFT JOIN [Decision].[dbo].[CreditBur7_Contract] a WITH (NOLOCK) on A.SubjectId = A1.Id
				WHERE	a1.rowid = @ROWID
				...
				SELECT	U.ROWID,
						...
						u1.dlamt
				FROM	[Decision].[dbo].[CreditBur3_comp] u WITH (NOLOCK)
				LEFT JOIN [Decision].[dbo].[CreditBur3_crdeal] u1 WITH (NOLOCK) ON u1.compId = u.Id
				INNER JOIN	(
								SELECT	T3.*,
										ROW_NUMBER() OVER (PARTITION BY T1.ROWID, T2.ID ORDER BY CAST(T3.DLYEAR AS VARCHAR(4))+RIGHT('00'+CAST(T3.DLMONTH AS VARCHAR(2)),2) DESC, T3.ID DESC)		AS	RN
								FROM	[Decision].[dbo].[CreditBur3_comp] T1 WITH (NOLOCK)
	...
	FROM	(
				...
				FROM	[Decision].[dbo].[CreditBur7_Subject] T1 WITH (NOLOCK)
				INNER JOIN [Decision].[dbo].[CreditBur7_Contract] T2 WITH (NOLOCK) ON T2.SubjectId = T1.ID
				WHERE	T1.RowId = @ROWID
			) V1;
	--	-----------------------------------------------------------------------------------------------------------------------------------
		...
		(
			...
			from	(
						...
						from	(
									...
									from	[Decision].[dbo].[CreditBur7_Subject] t1 WITH (NOLOCK)
									inner join [Decision].[dbo].[CreditBur7_Contract] t2 WITH (NOLOCK) on t2.SubjectId = t1.id
									left join [Decision].[dbo].[CreditBur7_Record] t3 WITH (NOLOCK) on t3.SubjectId = t1.id
    ...
    FROM	(
				SELECT CASE WHEN info IN('одружений (замiжня)', 'Одружений') THEN 'married'
						...
						CASE WHEN SUBSTRING(@INN, 9, 1) IN('0', '2', '4', '6', '8') THEN 'F'
							ELSE 'M'
						END			AS	SEX
				FROM	[Decision].dbo.[ConnotationValues] WITH (NOLOCK)
				WHERE	ownerid = @ROWID
						AND Kind = 876
			) V1;
	--	-----------------------------------------------------------------------------------------------------------------------------------
	...
	FROM	(
				...
				FROM	(
							SELECT	TOP 1
									ownerid
							FROM	[Decision].dbo.[ConnotationValues] a WITH (NOLOCK)
							WHERE	ownerid = @ROWID
						) h
				LEFT JOIN	(
								SELECT  ownerid ,
	...
	LEFT JOIN [RMDB].[Scoring].[ActivityField] b WITH (NOLOCK) ON l.info_1 = b.POSITION
	...
	;WITH CTE0 AS
		...
		(
			...
			FROM	[Decision].[dbo].[CreditBur3_comp] T1 WITH (NOLOCK)
			LEFT JOIN [Decision].[dbo].[CreditBur3_crdeal] T2 WITH (NOLOCK) ON T2.COMPID = T1.ID
																			AND T2.DLDONOR = 'MFO'
			LEFT JOIN [Decision].[dbo].[CreditBur3_deallife] T3 WITH (NOLOCK) ON T3.CRDEALID = T2.ID
			WHERE	T1.ROWID = @ROWID
			GROUP BY T1.ROWID
		)
	...
	BEGIN
		...
		SELECT	@ROWID,
				...
				'CALCULATION_TIME',
				NULL,
				CONVERT(VARCHAR(10), GETDATE()-@STARTCALCDATE, 108)
		UNION ALL
		SELECT	@ROWID,
				@INN,
				@isMBKIID,
				'CALCULATION_B2_TIME',
				NULL,
				CONVERT(VARCHAR(10), @ENDCALCDATE_B2-@STARTCALCDATE_B2, 108)
